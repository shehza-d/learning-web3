...
export async function POST(request: Request) {
    ...
    const user = (
        await dbClient.unsafe(
            `SELECT "Users".* from "Users" WHERE "publicAddress"='${publicAddress}'`,
        )
    )[0];
    ...
    await dbClient.unsafe(
        `UPDATE "Users" set nonce = '${Math.floor(
            Math.random() * 10000,
        ).toString()}', "secretText" = '${secretText}' WHERE "publicAddress" = '${publicAddress}'`,
    );
